...
class TestGitCommand:
    ...
    def test_run_failure(self, mock_check_output):
        mock_check_output.side_effect = CalledProcessError(1, 'git status', output="error output")
        cmd = GitCommand('status')
        result = cmd.run()
...
class TestGitOperations:
    ...
    def test_get_current_repo_slug(self, mock_check_output):
        mock_check_output.return_value = "origin\tgit@bitbucket.org:org/repo.git (fetch)\n"
